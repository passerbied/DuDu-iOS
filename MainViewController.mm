...
- (void)viewDidLoad {
    ...
    _TopToolBar = [[TopToolBar alloc] initWithFrame:ccr(0, 0, SCREEN_WIDTH, 50) carNames:@[@"不限",@"普通车",@"豪华车"]]; //TODO:可能为接口返回数据
...
-(void)onClickDriveSearch
{
    ...
    start.name = _bottomToolBar.fromAddressLabel.text;
    start.cityName = @"北京";
    BMKPlanNode* end = [[BMKPlanNode alloc]init];
    end.name = _bottomToolBar.toAddressLabel.text;
    end.cityName = @"北京";
...
- (void)didUpdateBMKUserLocation:(BMKUserLocation *)userLocation
{
    ...
    [_bottomToolBar updateLocation:userLocation.title];
}
    ...
    switch (routeAnnotation.type) {
        case 0:
        {
            ...
            if (view == nil) {
                ...
                view.image = IMG(@"path_mark_start");
        ...
        case 4:
        {
            ...
            UIImage* image = [UIImage imageWithContentsOfFile:[Utils getMyBundlePath1:@"images/icon_direction.png"]];
...
- (void)timePickerView:(TimePicker *)pickerView didSelectTime:(NSInteger)timeStamp
{
    ...
    NSLog(@"%@",[date displayWithFormat:@"yyyy年M月d日hh时mm分"]);
